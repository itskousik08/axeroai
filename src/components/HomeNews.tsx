...
export default function HomeNews() {
  // We take the first 3 news items from the JSON
  const latestNews = newsData.slice(0, 3);

  return (
    <section className="py-24 relative overflow-hidden group/section">
      {/* Decorator */}
      <div className="absolute top-0 right-0 w-1/2 h-full bg-primary/2 skew-x-12 translate-x-1/2 pointer-events-none transition-transform duration-1000 group-hover/section:translate-x-[40%]"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-20">
          <div className="space-y-4">
            ...
            >
              ...
              <span className="section-label !mb-0 tracking-[0.3em]">Intelligence Briefing</span>
            </motion.div>
            <motion.h2 
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            <a href="/news" className="premium-button premium-button-secondary group/btn">
              Explore Newsroom
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
          {latestNews.map((news, i) => (
            ...
            >
              <a href={`/news/${news.slug}`} className="group block h-full">
                {news.image ? (
                  <div className="relative aspect-video rounded-2xl md:rounded-3xl overflow-hidden border border-outline bg-surface-accent shadow-xl transition-all duration-500 group-hover:shadow-primary/15 group-hover:border-primary/40 mb-8">
                    <img 
                      src={news.image} 
                      alt={news.title} 
                    ...
                    <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 flex items-center justify-between opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                      <div className="flex items-center gap-2 text-[9px] md:text-[10px] font-mono text-white uppercase tracking-widest bg-primary/20 backdrop-blur-sm px-3 py-1 rounded-full">
                ...
                <div className="space-y-4 px-1">
                  <div className="flex items-center gap-2 text-[10px] font-mono text-on-surface-muted uppercase tracking-[0.2em]">
                    <span>{new Date(news.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span>
                  </div>
